...
def main():
    ...
    values_scaler = StandardScaler(with_std=True)
    values_train_shifted = values_scaler.fit_transform(values_train)
    values_test_shifted = values_scaler.fit_transform(values_test)
    ...
    #regression_grid_searches(training_features=values_train_shifted,training_labels=hdi_train,
    #        testing_features=values_test_shifted)

    # Part i, m, : Nearest neighbors to the US. n_neighbors is 8 because the first entry is the US.
    #nbrs = NearestNeighbors(n_neighbors=8).fit(values_train_shifted)
    #distances, indices = nbrs.kneighbors([values_train_shifted[45]])
...
def _rmse_grid_search(training_features, training_labels, pipeline, parameters, technique):
    ...
    Output:
        ...
        Plots RMSE vs k for k kearest Neighbors Regression
    """
